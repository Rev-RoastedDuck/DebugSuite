...
// \def 0: not show color  1: show color
...
/** \addtogroup debug commom
 ** \{ */
#define __DEBUG_PRINT( __fmt, ...) 	printf(__fmt, ##__VA_ARGS__)
/** \} */
...
#define DEBUG_PRINT_COLOR_BG_BRIGHT_WHITE   107
		...
		#define DEBUG_PRINT(__enable, __fmt, ...)  											\
			do {  																			\
				if (__enable) {  															\
					__DEBUG_PRINT("[%s:%d] " __fmt, __FILENAME__, __LINE__, ##__VA_ARGS__); \
				}  																			\
			} while (0)
	#else
		#undef DEBUG_PRINT
		#define DEBUG_PRINT(__enable, __fmt, ...)  			\
			do {  											\
				if (__enable) {  							\
					__DEBUG_PRINT(__fmt, ##__VA_ARGS__); 	\
				}  \
			} while (0)
	#endif
		...
		#define DEBUG_PRINT_COLOR(__enable, __font_attr,__fg_color, __bg_color, __fmt, ...) 	\
			do { 																				\
				if(__enable){ 																	\
					__DEBUG_PRINT("\033[");														\
					__font_attr >= 0 && (__DEBUG_PRINT("%d", 	__font_attr),1); 				\
					__fg_color  >= 0 && (__DEBUG_PRINT(";%d",  __fg_color), 1); 				\
					__bg_color  >= 0 && (__DEBUG_PRINT(";%d",  __bg_color), 1); 				\
					__DEBUG_PRINT("m");															\
					DEBUG_PRINT(__enable,__fmt,##__VA_ARGS__);									\
					__DEBUG_PRINT("\033[0m\r\n");												\
				}																				\
			} while (0)
	...
	#define DEBUG_PRINT(__enable, __fmt, ...)
	#define DEBUG_PRINT_COLOR(__enable, __font_attr,__fg_color, __bg_color, __fmt, ...)
	#define DEBUG_PRINT_DEBUG(__enable,fmt, ...)   
	#define DEBUG_PRINT_INFO(__enable,fmt, ...)    
	#define DEBUG_PRINT_NOTICE(__enable,fmt, ...)  
	#define DEBUG_PRINT_WARNING(__enable,fmt, ...) 
	#define DEBUG_PRINT_ERROR(__enable,fmt, ...)   
	#define DEBUG_PRINT_FATAL(__enable,fmt, ...)   
	#define DEBUG_PRINT_SUCCESS(__enable,fmt, ...) 
#endif
/** \} */
